...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print "please input info.plist"
    fh = open(sys.argv[1], "rb")
    data = fh.read()
    fh.close()
    reader = BPlistReader(data)
    print reader.parse()
    # > decoder.py info.plist
    
    # {'CFBundleInfoDictionaryVersion': '6.0', 'DTXcodeBuild': '4C199', 'CFBundleSuppo
    # rtedPlatforms': ['iPhoneOS'], 'CFBundleIdentifier': 'com.appsec-labs.inalyzer5',
    #  'DTSDKName': 'iphoneos5.0', 'DTSDKBuild': '9A334', 'CFBundleDisplayName': 'iNal
    # yzer5', 'BuildMachineOSBuild': '10K549', 'CFBundleExecutable': 'iNalyzer', 'Mini
    # mumOSVersion': '3.0', 'CFBundleVersion': '1.0', 'UIDeviceFamily': [1], 'DTPlatfo
    # rmBuild': '9A334', 'NSMainNibFile': 'MainWindow', 'DTXcode': '0420', 'CFBundleDe
    # velopmentRegion': 'English', 'DTPlatformName': 'iphoneos', 'DTCompiler': 'com.ap
    # ple.compilers.llvm.clang.1_0', 'CFBundleSignature': '????', 'LSRequiresIPhoneOS'
    # : True, 'DTPlatformVersion': '5.0', 'CFBundleName': 'iNalyzer5', 'CFBundlePackag
    # eType': 'APPL'}
